...
function InitModConfigMenu()
    ...
    if ModConfigMenu then
        ...
        ModConfigMenu.AddText(categoryName, "Whitelist", function() return "Whitelist Specific Pickups" end)

        ModConfigMenu.AddSpace(categoryName, "Whitelist")

        ModConfigMenu.AddSetting(categoryName, "Whitelist",
        {
            Type = ModConfigMenu.OptionType.BOOLEAN,
            CurrentSetting = function ()
                return ReactionPack.Settings.ReactToCrane
            end,
            Display = function()
                local choice = ReactionPack.MCMStrings[ReactionPack.Settings.ReactToCrane]
                return "Crane Games" .. ': ' .. choice
            end,
            OnChange = function(currentSetting)
                ReactionPack.Settings.ReactToCrane = currentSetting
            end,
            Info = {"If True, Isaac will react to the Items contained within Crane Games"}
        })
        ...
        ModConfigMenu.AddSetting(categoryName, "Whitelist",
        {
            Type = ModConfigMenu.OptionType.BOOLEAN,
            CurrentSetting = function ()
                return ReactionPack.Settings.ReactToFlip
            end,
            Display = function()
                local choice = ReactionPack.MCMStrings[ReactionPack.Settings.ReactToFlip]
                return "Flip Items" .. ': ' .. choice
            end,
            OnChange = function(currentSetting)
                ReactionPack.Settings.ReactToFlip = currentSetting
            end,
            Info = {"If True, Isaac will react to the Additional Item generated by Flip"}
        })
